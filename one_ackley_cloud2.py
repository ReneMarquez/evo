...
for i in range(dict["NUMBER_OF_WORKERS"]):
        dict1 = {'NUMBER_OF_WORKERS':range(dict["NUMBER_OF_WORKERS"]),'SAMPLE_SIZE':config['SAMPLE_SIZE'],'POPULATION_SIZE':config['POPULATION_SIZE'],'SERVER': 'http://54.164.20.127:5000/evospace','WORKER_TYPE': 'c1','CHROMOSOME_LENGTH': config["CHROMOSOME_LENGTH"],'PEAKS': 512,'WORKER_GENERATIONS': config["WORKER_GENERATIONS"],'MAX_SAMPLES': config["MAX_SAMPLES"],'MUTATION_FLIP_PB': .05,'TOURNAMENT_SIZE': 4,'CXPB':round(random.uniform(0.5,1),2),'MUTPB':round(random.uniform(0,0.1),3),'RETURN_RATE': 1.00}
        dic.append(dict1)

datafile = open("EvoPar2015/code/data/one_ackley_n1-"+experiment_id+".dat","a")
conf_out = open("EvoPar2015/code/conf/one_ackley_n1-"+experiment_id+".yaml","w")
